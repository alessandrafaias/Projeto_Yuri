from os import walk
import os
#import shutil
import re

f = []
for (dirpath, dirnames, filenames) in walk("C:/Users/user/Desktop/PROJETO/ARQUIVOS"):
    for file in filenames:
    	f.append(file)

print ("Listando arquivos a serem transferidos \n")
print (dirpath, f)
print("\nQuantidade de arquivos encontrados:", +len(f) , "\n")
i = 0
while i < len(f):	
	os.rename('C:/Users/user/Desktop/PROJETO/ARQUIVOS/'+f[i] , 'C:/Users/user/Desktop/PROJETO/OUTPUT/'+f[i].replace(".txt",".hql"))
	#print ('resultado')
	#print (f[i])
	i += 1
fd = []
for (dirpath, dirnames, filenames) in walk("C:/Users/user/Desktop/PROJETO/OUTPUT"):
    for file in filenames:
    	fd.append(file)


print ("Listando arquivos após transferência \n")
print (dirpath, fd)

pattern = re.pattern(r'ale')

matches = pattern.finditer(alessandra)

for match in matches
	print(mahtch)